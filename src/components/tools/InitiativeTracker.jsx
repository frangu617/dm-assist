...
import { Grid, Button, Card, CardContent, List, ListItem, ListItemText, TextField, Typography, Box } from '@mui/material';
...
const InitiativeTracker = () => {
    ...
    const handleAddCharacter = (e) => {
        e.preventDefault();
        const newCharacter = { name: newCharacterName, initiativeOrder: parseInt(newCharacterInitiative) };
        setInitiativeQueue([...initiativeQueue, newCharacter].sort((a, b) => b.initiativeOrder - a.initiativeOrder));
        setNewCharacterName('');
    ...
    const renderQueue = () => (
        <List>
            {initiativeQueue.map((character, index) => (
                <ListItem key={index}>
                    <ListItemText primary={`${character.name} (Initiative: ${character.initiativeOrder})`} />
                </ListItem>
            ))}
        </List>
    );
    ...
    return (
        <Box sx={{ maxWidth: 500, margin: 'auto', mt: 4 }}>
            ...
            <Grid container spacing={2} justifyContent="center">
                ...
                <Grid item>
                    <Button variant="outlined" color="secondary" onClick={handleClearQueue}>
                        Clear Queue
                    </Button>
                </Grid>
            </Grid>
